...
>
> Если цель — не store-клиент, а **сборка из этого репозитория** на Intel Mac /
> личном Apple Team, см. [`IOS_LOCAL_BUILD_RU.md`](IOS_LOCAL_BUILD_RU.md). Это
> отдельное приложение (`com.pawga.fluffychat` / FluffyChat Dev), рядом с
> официальным из App Store.
